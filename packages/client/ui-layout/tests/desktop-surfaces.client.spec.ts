...
describe('AppFrame desktop surfaces', () => {
  ...
  it('uses theme tokens for the desktop tint and opaque work columns', () => {
    expect(css).toContain('--dsw-specific-sidebar-column-fill,')
    expect(css).toContain('--dsw-specific-sidebar-column-divider,')
    expect(css).toContain('color-mix(in srgb, var(--dsw-specific-sidebar-fill) 72%, transparent)')
    expect(css).toContain(
      'background: var(--dsw-specific-center-column-fill, var(--dsw-alias-bg-base));',
    )
  })
})
